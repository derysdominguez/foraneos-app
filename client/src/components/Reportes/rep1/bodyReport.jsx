...
function BodyReport1() {
  ...
  const api = async () => {
    try {
      ...
      const datosSinEntradaIngresos = ingresos.map(
        ({ entrada, ...resto }) => resto
      )
      const totalIngresos = ingresos.reduce(
        (acum, ingresos) => parseFloat(acum) + parseFloat(ingresos.monto),
        0
      )

      const egresos = lbJSON.filter((item) => item.entrada == 'e')
      const datosSinEntradaEngresos = egresos.map(
        ({ entrada, ...resto }) => resto
      )
      const totalEgresos = egresos.reduce(
        (acum, egresos) => parseFloat(acum) + parseFloat(egresos.monto),
        0
      )
  ...
  return (
    <div className=' hsizing-control w-100 h-50 overflow-auto p-2 d-flex flex-wrap rounded align-items-center justify-content-center'>
      <div className=' col-12 mt-1 h-100'>
        ...
        <hr></hr>
        <TableGenerator
          data={egresos ? egresos : [{ Estado: 'Cargando...' }]}
        />
        <Table striped bordered>
          <tbody>
            <tr className='bg-secondary'>
